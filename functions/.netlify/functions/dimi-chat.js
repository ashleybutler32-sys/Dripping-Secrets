// ─────────────────────────────────────────────────────────────────────────────
//  DIMI — Jarvis Mode AI Engine (Cloudflare Pages Function)
...
export async function onRequest(context) {
  ...
  if (!apiKey) {
    return new Response(JSON.stringify({
      reply: `I'm in standby mode right now, love — my AI brain needs an OpenAI API key to fire up. Ashley, head to Cloudflare Pages → Settings → Environment variables → add OPENAI_API_KEY, then redeploy. Once that's in, I'm fully operational. 💜`,
      fallback: true
    }), { status: 200, headers });
  }
  ...
  const { message, mode = 'customer', history = [], context: chatContext = {} } = body;
  ...
  if (mode === 'customer') {
    const parts = [];
    if (chatContext.pageContext) parts.push(`Location: ${chatContext.pageContext}`);
    if (chatContext.category)    parts.push(`Active category filter: ${chatContext.category}`);
    if (chatContext.product)     parts.push(`Product they are viewing: ${chatContext.product}`);
    if (chatContext.cartItems)   parts.push(`Items in cart: ${chatContext.cartItems}`);
    if (parts.length > 0) {
  ...
  if (mode === 'admin' && Object.keys(chatContext).length > 0) {
    const parts = [];
    if (chatContext.pendingOrders !== undefined)  parts.push(`Pending orders: ${chatContext.pendingOrders}`);
    if (chatContext.totalRevenue !== undefined)   parts.push(`Total revenue (all time): $${parseFloat(chatContext.totalRevenue).toFixed(2)}`);
    if (chatContext.todayRevenue !== undefined)   parts.push(`Today's revenue: $${parseFloat(chatContext.todayRevenue).toFixed(2)}`);
    if (chatContext.lowStockItems?.length)        parts.push(`Low stock items (≤3 units): ${chatContext.lowStockItems.join(', ')}`);
    if (chatContext.recentOrders?.length) {
      parts.push(`Recent orders (last 5): ${chatContext.recentOrders.map(o =>
        `Order #${o.id} — $${parseFloat(o.total||0).toFixed(2)} — ${o.status} — ${o.customerName || 'Guest'}`
      ).join(' | ')}`);
    }
    if (chatContext.physicalInventory?.length) {
      parts.push(`Physical inventory: ${chatContext.physicalInventory.map(i =>
        `${i.name}: ${i.qty} units`
      ).join(', ')}`);
    }
    if (parts.length > 0) {
  ...
  if (mode === 'content_studio' && Object.keys(chatContext).length > 0) {
    const parts = [];
    if (chatContext.theme)         parts.push(`Today's theme: ${chatContext.theme}`);
    if (chatContext.platform)      parts.push(`Target platform: ${chatContext.platform}`);
    if (chatContext.contentType)   parts.push(`Content type: ${chatContext.contentType}`);
    if (chatContext.product?.name) parts.push(`Featured product: ${chatContext.product.name} — ${chatContext.product.description || ''}`);
    if (chatContext.dayOfWeek)     parts.push(`Day of week: ${chatContext.dayOfWeek}`);
    if (parts.length > 0) {
  ...
  try {
    ...
    if (mode === 'content_studio') {
      try {
        ...
        return new Response(JSON.stringify({ content: parsed, fallback: false }), { status: 200, headers });
      } catch {
        return new Response(JSON.stringify({
    ...
    return new Response(JSON.stringify({ reply, fallback: false }), { status: 200, headers });
