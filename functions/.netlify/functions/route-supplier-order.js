// ─────────────────────────────────────────────────────────────────────────────
//  route-supplier-order.js — Dripping Secrets DS-DOS v18.2 (Cloudflare Pages Function)
...
async function sendEmail(brevoApiKey, to, cc, subject, htmlBody) {
...
export async function onRequest(context) {
  ...
  const projectId = FIREBASE_SERVICE_ACCOUNT ? JSON.parse(FIREBASE_SERVICE_ACCOUNT).project_id : null;
  ...
  if (supplierKey === 'none' || supplierKey === 'ds') {
    await updateOrderStatus(FIREBASE_SERVICE_ACCOUNT, projectId, orderId, { status: 'ds_fulfilled', supplierKey: 'none' });
    return new Response(JSON.stringify({ routed: false, reason: 'DS-held inventory — no supplier routing needed' }), { status: 200, headers: CORS });
  }
  ...
  if (!supplier) {
    ...
    try {
      await sendEmail(BREVO_API_KEY, MANAGEMENT_EMAIL, null, `⚠️ DS Order Routing Alert — ${orderId}`, buildAlertEmailBody(order, alertMsg));
    } catch (emailErr) {
      console.error('[route-supplier-order] Alert email failed:', emailErr.message);
    }
    await updateOrderStatus(FIREBASE_SERVICE_ACCOUNT, projectId, orderId, { status: 'routing_failed', routingError: alertMsg });
    await appendRoutingLog(FIREBASE_SERVICE_ACCOUNT, projectId, { orderId, supplierKey: supplierKey || 'unknown', routed: false, reason: 'unknown_supplier', method: order.paymentMethod || 'unknown' });
    return new Response(JSON.stringify({ routed: false, alert: true, reason: alertMsg }), { status: 200, headers: CORS });
  }

  // ── Route order to supplier ──
  try {
    const subject = `New Dropship Order — ${orderId} [Dripping Secrets]`;
    await sendEmail(BREVO_API_KEY, supplier.email, MANAGEMENT_EMAIL, subject, buildOrderEmailBody(order, supplier));

    // ── Update Firestore order status ───────────────────────────────────────
    await updateOrderStatus(FIREBASE_SERVICE_ACCOUNT, projectId, orderId, {
    ...
    await appendRoutingLog(FIREBASE_SERVICE_ACCOUNT, projectId, {
      orderId,
      supplierKey,
    ...
    try {
      ...
      await sendEmail(BREVO_API_KEY, MANAGEMENT_EMAIL, null, `⚠️ DS Order Routing FAILED — ${orderId}`, buildAlertEmailBody(order, alertMsg));
    } catch (alertErr) {
      console.error('[route-supplier-order] Alert email also failed:', alertErr.message);
    }

    return new Response(JSON.stringify({ routed: false, error: err.message, orderId }), { status: 500, headers: CORS });
  }
}
